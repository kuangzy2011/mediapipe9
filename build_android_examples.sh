...
#app="${app_dir}/basic"
#app="${app_dir}/handtrackinggpu"
#app="${app_dir}/holistictrackinggpu"
#app="${app_dir}/facemeshgpu"
app="${app_dir}/holistictrackinggpu"
echo "=== app: ${app}"
app_name=${app##*/}
...
if [[ ${app_name} == "objectdetection3d" ]]; then
  ...
  for category in "${categories[@]}"; do
    ...
    if [[ $install_only == false ]]; then
      ...
      bazelisk "${bazel_flags_extended[@]}"
      echo ">>compile flags1: ${bazel_flags_extended[@]}"
      cp -f "${bin}" "${apk}"
    fi
    apks+=(${apk})
  done
  ...
  if [[ $install_only == false ]]; then
    ...
    echo ">>compile flags2: ${bazel_flags[@]}"
    bazelisk "${bazel_flags[@]}"
    cp -f "${bin}" "${apk}"
    if [[ ${app_name} == "templatematchingcpu" ]]; then
      switch_to_opencv_3
