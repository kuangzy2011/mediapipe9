...
#include "mediapipe/calculators/tensor/landmarks_list_to_tensor_calculator.h"

#include <memory>

#include "mediapipe/calculators/tensor/landmarks_list_to_tensor_calculator.pb.h"
...
float GetAttribute(const Landmark& landmark, const LandmarksListToTensorCalculatorOptions::Attribute& attribute) {
  switch (attribute) {
    case LandmarksListToTensorCalculatorOptions::X:
      return landmark.x();
    case LandmarksListToTensorCalculatorOptions::Y:
      return landmark.y();
    case LandmarksListToTensorCalculatorOptions::Z:
      return landmark.z();
    case LandmarksListToTensorCalculatorOptions::VISIBILITY:
      return landmark.visibility();
    case LandmarksListToTensorCalculatorOptions::PRESENCE:
...
class LandmarksListToTensorCalculatorImpl: public NodeImpl<LandmarksListToTensorCalculator> {
 public:
  absl::Status Open(CalculatorContext* cc) override {
    options_ = cc->Options<LandmarksListToTensorCalculatorOptions>();
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    //LOG(INFO) << ">>LandmarksListToTensorCalculator n_landmarks " << n_landmarks << ", c_size " << c_size;
    ...
    //LOG(INFO) << ">>>>>>>>>>>>>>>>>>LandmarksListToTensorCalculator num hands: " << c_size;
 ...
 private:
  LandmarksListToTensorCalculatorOptions options_;
  struct Connection {
    int start;
    int end;
  };
...
MEDIAPIPE_NODE_IMPLEMENTATION(LandmarksListToTensorCalculatorImpl);
